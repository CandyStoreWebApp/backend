...
namespace SweetIncApi.Controllers
{
    ...
    public class OriginsController : ControllerBase
    {
        ...
        [HttpPost("with_query")]
        public IActionResult GetAll(OriginPagingVM queries)
        {
            try
            {
                return Ok(_originRepository.GetAll(queries));
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, ex);
        ...
        [HttpPut("{id}")]
        public IActionResult Update(int id, OriginUpdateVM updateOrigin)
        {
